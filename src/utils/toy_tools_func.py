...
def get_N_eff(sampler, args, num_chains = 10, mc_iter = 100):
    params_chains = []
    # Vérifier a réinitialisation.
    for _ in range(num_chains):
        salad = sampler(*args)
        # run the algorithm
        for _ in range(mc_iter):
            salad.step()
        params_chains.append(salad.get_saved_params())
    min_size = np.min([len(par_ch) for par_ch in params_chains])
    reduced_chains = []
    for par_ch in params_chains:
        ind = np.random.choice(len(par_ch), size=min_size, replace=False)
        reduced_chains.append(par_ch[ind])
    reduced_chains = np.squeeze(reduced_chains)
    # ess = ESS(reduced_chains)
    # N_eff = ess.get_N_eff()
    # print('Sophisticated method gives: N_eff / N = %s' % (N_eff / reduced_chains.shape[1]))
